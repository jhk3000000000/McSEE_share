...
//************************************** TEST **************************************//
void ETHuman3DApp::test_obj()
{
	m_pVTKWidget->GetSceneRenderer()->RemoveActor(testActor1);
	m_pVTKWidget->GetSceneRenderer()->RemoveActor(testActor2);
	m_pVTKWidget->GetSceneRenderer()->RemoveActor(testActor3);
	testActor1 = nullptr;	
	testActor2 = nullptr;
	testActor3 = nullptr;
	...
	double startAzimuthalAngle = theApp.pRt->PhantomPosY_QLineEdit->text().toDouble();
	double deltaAzimuthalAngle = theApp.pRt->PhantomRotY_QLineEdit->text().toDouble();	
	double AzimuthalIntervalCount = std::ceil(deltaAzimuthalAngle);
	double startPolarAngle = theApp.pRt->PhantomPosZ_QLineEdit->text().toDouble();
	double deltaPolarAngle = theApp.pRt->PhantomRotZ_QLineEdit->text().toDouble();
	...
	//theApp.SetMessageBox("PolarSeparationCount: " + QString::number(PolarSeparationCount) + "\nAzimuthalSeparationCount: " + QString::number(AzimuthalSeparationCount));
	...
	for (int PolarSeparation = 0; PolarSeparation < PolarSeparationCount; PolarSeparation++)
	{
		...
		for (int AzimuthalSeparation = 0; AzimuthalSeparation < AzimuthalSeparationCount; AzimuthalSeparation++)
		{
			...
			//theApp.SetMessageBox("PolarSeparation: " + QString::number(PolarSeparation) + "\nAzimuthalSeparation: " + QString::number(AzimuthalSeparation));
			//theApp.SetMessageBox("SeparationStartPolarAngle: " + QString::number(SeparationStartPolarAngle) + "\nSeparationDeltaPolarAngle: " + QString::number(SeparationDeltaPolarAngle));
			//theApp.SetMessageBox("SeparationStartAzimuthalAngle: " + QString::number(SeparationStartAzimuthalAngle) + "\nSeparationDeltaAzimuthalAngle: " + QString::number(SeparationDeltaAzimuthalAngle));
	...
	double OuterRadius = theApp.pRt->PhantomRotX_QLineEdit->text().toDouble();
	...
	outerSpherePolyData = transformFilter1->GetOutput();
		
	double InnerRadius = theApp.pRt->PhantomPosX_QLineEdit->text().toDouble();
	...
	InnerSpherePolyData = transformFilter2->GetOutput();

	//Generate mesh of inner surface of partial sphere	
	std::vector<vtkSmartPointer<vtkPolyData>> GeneratedCuttedPolyData;
	double normarlizedOuterRadius = OuterRadius / InnerRadius;
	double normarlizedInnerRadius = 1.0;
	...
	{
		std::vector<vtkSmartPointer<vtkPolyData>> GeneratedPolyData;
		...
		for (int radialInterval = 0; radialInterval < radialIntervalCount; radialInterval++)
		{
			...
			delaunay->SetInputData(polyData);
			double alphaFactor = 3; // Adjust this
			delaunay->SetAlpha(2 * M_PI*radialIntervalInnerRadius / 360 * alphaFactor); // Need for optimized (~0.6 for minimum radius)
			delaunay->Update();

			vtkSmartPointer<vtkTransform> transform = vtkSmartPointer<vtkTransform>::New();
			transform->PostMultiply(); //this is the key line
			transform->Scale(InnerRadius, InnerRadius, 1.0);
			...
			GeneratedPolyData.push_back(transformFilter->GetOutput());
		}
		vtkSmartPointer<vtkPolyData> AppendedPolyData = GeneratedPolyData[0];
		for (int radialInterval = 0; radialInterval < radialIntervalCount - 1; radialInterval++)
		{
			vtkSmartPointer<vtkAppendFilter> appendFilter = vtkSmartPointer<vtkAppendFilter>::New();
			appendFilter->AddInputData(AppendedPolyData);
			appendFilter->AddInputData(GeneratedPolyData[radialInterval + 1]);
			appendFilter->Update();
		...
		GeneratedCuttedPolyData.push_back(AppendedPolyData);
	}
	if (endAzimuthalUsed)
	{
		std::vector<vtkSmartPointer<vtkPolyData>> GeneratedPolyData;
	...
	if (startPolarUsed)
	{
		...
		for (int radialInterval = 0; radialInterval < radialIntervalCount; radialInterval++) // Radial part (1 cm 두께)
		{
			...
			//theApp.SetMessageBox("radialInterval: " + QString::number(radialInterval) + "/" + QString::number(radialIntervalCount)
			//	+ "\nradialIntervalOuterRadius: " + QString::number(radialIntervalOuterRadius)
			//	+ "\nradialIntervalInnerRadius: " + QString::number(radialIntervalInnerRadius));
			...
			delaunay->SetInputData(polyData);
			double alphaFactor = 3; // Adjust this
			delaunay->SetAlpha(2 * M_PI*radialIntervalInnerRadius / 360 * alphaFactor);
			delaunay->Update();

			vtkSmartPointer<vtkTransform> transform = vtkSmartPointer<vtkTransform>::New();
			transform->PostMultiply(); //this is the key line
			transform->Scale(InnerRadius, InnerRadius, InnerRadius);
			vtkSmartPointer<vtkTransformPolyDataFilter> transformFilter = vtkSmartPointer<vtkTransformPolyDataFilter>::New();
			transformFilter->SetInputData(delaunay->GetOutput());
			transformFilter->SetTransform(transform);
			transformFilter->Update();
		...
		AppendedPolyData = GeneratedPolyData[0];
		for (int radialInterval = 0; radialInterval < radialIntervalCount - 1; radialInterval++)
		{
			vtkSmartPointer<vtkAppendFilter> appendFilter = vtkSmartPointer<vtkAppendFilter>::New();
			appendFilter->AddInputData(AppendedPolyData);
	...
	// vtkDecimatePro to Decimate generated PolyData
	//vtkSmartPointer<vtkDecimatePro> decimate = vtkSmartPointer<vtkDecimatePro>::New();
	//decimate->SetInputData(AppendedCuttedPolyData);
	//decimate->SetTargetReduction(0.2);
	//decimate->PreserveTopologyOn(); // Preserve the topology
	//decimate->BoundaryVertexDeletionOff(); // Attempt to preserve boundary edges
	//decimate->Update();
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (i == 0)
		{
			mapper->SetInputData(outerSpherePolyData);
			testActor1 = vtkSmartPointer<vtkActor>::New();
			testActor1->SetMapper(mapper);
			testActor1->SetProperty(property);
		}
		if (i == 1)
		{
			mapper->SetInputData(InnerSpherePolyData);
			testActor2 = vtkSmartPointer<vtkActor>::New();
			testActor2->SetMapper(mapper);
			testActor2->SetProperty(property);
		}
		if (i == 2)
		{
			mapper->SetInputData(AppendedCuttedPolyData);
			testActor3 = vtkSmartPointer<vtkActor>::New();
			testActor3->SetMapper(mapper);
			testActor3->SetProperty(property);
		}
	}
	m_pVTKWidget->GetSceneRenderer()->AddActor(testActor1);
	m_pVTKWidget->GetSceneRenderer()->AddActor(testActor2);
	m_pVTKWidget->GetSceneRenderer()->AddActor(testActor3);
	//m_pVTKWidget->renderWindow()->Render();
	m_pVTKWidget->renderWindow()->Render(); // VTK의 Qt wrapper이니까 Get 사용 안함

}
void ETHuman3DApp::test_poly()
{	
	SetMessageBox("123");
}
...
//************************************** Geometry Widget **************************************//
void ETHuman3DApp::AddAzimuthalAngularPoints(vtkSmartPointer<vtkPoints> points, double radius, double startAngle, double endAngle, double FixedAngle, int numPoints) {
...
void ETHuman3DApp::AddPartialCirclePoints(vtkSmartPointer<vtkPoints> points, double radius, double startAngle, double endAngle, int numPoints) {
	double deltaAngle = (endAngle - startAngle) / (numPoints - 1);
	for (int i = 0; i < numPoints; ++i) {
...
void ETHuman3DApp::ObjectGenerate_Box(int GeometryObjectID)
{	
	bool isUpdate = false;
	if (ObjectPanelActors[GeometryObjectID] != nullptr) // 기존에 존재해서 update 상황일때
	{
		isUpdate = true;
		m_pVTKWidget->GetSceneRenderer()->RemoveActor(ObjectPanelActors[GeometryObjectID]);
		ObjectPanelActors[GeometryObjectID] = nullptr; // ObjectActor 삭제
		m_pVTKWidget->renderWindow()->Render();
		ObjectPanelPolydata_Collection[GeometryObjectID] = nullptr; // ObjectPolydata 삭제
	}

	// Get global variables 
	double XCenter = theApp.pRt->m_Object_MainInfo[GeometryObjectID][1];
	double YCenter = theApp.pRt->m_Object_MainInfo[GeometryObjectID][2];
	double ZCenter = theApp.pRt->m_Object_MainInfo[GeometryObjectID][3];
	double XRotate = theApp.pRt->m_Object_MainInfo[GeometryObjectID][4];
	double YRotate = theApp.pRt->m_Object_MainInfo[GeometryObjectID][5];
	double ZRotate = theApp.pRt->m_Object_MainInfo[GeometryObjectID][6];
	double HalfLenngthX = theApp.pRt->m_Object_MainInfo[GeometryObjectID][7];
	double HalfLenngthY = theApp.pRt->m_Object_MainInfo[GeometryObjectID][8];
	double HalfLenngthZ = theApp.pRt->m_Object_MainInfo[GeometryObjectID][9];
	double ThicknessX = theApp.pRt->m_Object_MainInfo[GeometryObjectID][10];
	double ThicknessY = theApp.pRt->m_Object_MainInfo[GeometryObjectID][11];
	double ThicknessZ = theApp.pRt->m_Object_MainInfo[GeometryObjectID][12];
	...
	ObjectPanelPolydata_Collection[GeometryObjectID] = AppendedPolyData;

	// Visualize
	vtkSmartPointer<vtkPolyDataMapper> mapper = vtkSmartPointer<vtkPolyDataMapper>::New();
	mapper->SetInputData(ObjectPanelPolydata_Collection[GeometryObjectID]);
	vtkSmartPointer<vtkActor> actor = vtkSmartPointer<vtkActor>::New();
	actor->SetMapper(mapper);
	ObjectPanelActors[GeometryObjectID] = actor;

	ObjectPanelActors[GeometryObjectID]->SetProperty(Util::GetProperty());
	ObjectPanelActors[GeometryObjectID]->GetProperty()->SetColor(0.5, 0.5, 0.5);

	if(isUpdate) ObjectPanelActors[GeometryObjectID]->GetProperty()->SetOpacity(1 - (double(pRt->m_Object_Transparency[GeometryObjectID]) / 100));
	else ObjectPanelActors[GeometryObjectID]->GetProperty()->SetOpacity(0.5);
	ObjectPanelActors[GeometryObjectID]->SetPosition(XCenter, YCenter, ZCenter);
	ObjectPanelActors[GeometryObjectID]->SetOrientation(XRotate, YRotate, ZRotate);

	m_pVTKWidget->GetSceneRenderer()->AddActor(ObjectPanelActors[GeometryObjectID]);
	m_pVTKWidget->renderWindow()->Render();
}
void ETHuman3DApp::ObjectGenerate_Sphere(int GeometryObjectID)
{
	bool isUpdate = false;
	if (ObjectPanelActors[GeometryObjectID] != nullptr)
	{
		isUpdate = true;
		m_pVTKWidget->GetSceneRenderer()->RemoveActor(ObjectPanelActors[GeometryObjectID]);
		ObjectPanelActors[GeometryObjectID] = nullptr; // ObjectActor 삭제
	...
	double YRotate = theApp.pRt->m_Object_MainInfo[GeometryObjectID][5];
	double ZRotate = theApp.pRt->m_Object_MainInfo[GeometryObjectID][6];
	double InnerRadius = theApp.pRt->m_Object_MainInfo[GeometryObjectID][14];
	double OuterRadius = theApp.pRt->m_Object_MainInfo[GeometryObjectID][15];
	double AzimuthalStartingAngle = theApp.pRt->m_Object_MainInfo[GeometryObjectID][16];
	double AzimuthalDeltaAngle = theApp.pRt->m_Object_MainInfo[GeometryObjectID][17];	
	double AzimuthalEndAngle = AzimuthalStartingAngle + AzimuthalDeltaAngle;
	double PolarStartingAngle = theApp.pRt->m_Object_MainInfo[GeometryObjectID][18];
	double PolarDeltaAngle = theApp.pRt->m_Object_MainInfo[GeometryObjectID][19];		
	...
	if (InnerRadius == 0 && AzimuthalDeltaAngle == 360 && PolarDeltaAngle == 180)
	{
		...
		transformFilter1->SetTransform(transform1);
		transformFilter1->Update();
		ObjectPanelPolydata_Collection[GeometryObjectID] = transformFilter1->GetOutput();
	}	
...
vtkSmartPointer<vtkPolyData> ETHuman3DApp::CreateTubeMesh(double R_in, double R_out, double halfH, double startDeg, double deltaDeg, int resolution)
{
...
void ETHuman3DApp::ObjectGenerate_Cylinder(int GeometryObjectID)
{
	...
	if (ObjectPanelActors[GeometryObjectID] != nullptr) {
		isUpdate = true;
		m_pVTKWidget->GetSceneRenderer()->RemoveActor(ObjectPanelActors[GeometryObjectID]);
		ObjectPanelActors[GeometryObjectID] = nullptr;
		m_pVTKWidget->renderWindow()->Render();
		ObjectPanelPolydata_Collection[GeometryObjectID] = nullptr;
	}

	// 2) 파라미터 로드
	auto& info = theApp.pRt->m_Object_MainInfo[GeometryObjectID];
	...
	// 4) 저장
	ObjectPanelPolydata_Collection[GeometryObjectID] = pd;
	...
	actor->GetProperty()->SetOpacity(
		isUpdate
		? 1.0 - (pRt->m_Object_Transparency[GeometryObjectID] / 100.0)
		: 0.5
	);
	...
	ObjectPanelActors[GeometryObjectID] = actor;
	m_pVTKWidget->GetSceneRenderer()->AddActor(actor);
	m_pVTKWidget->renderWindow()->Render();
}

void ETHuman3DApp::UpdateObject_InfoStatus_InActorMouseControl(int objectIndex) // Actor의 데이터를 InfoData에 저장
{
	int id = pRt->m_Object_SelectedIndex;

	double* pCenter = ObjectPanelActors[id]->GetPosition();
	double TransformPos[3];
	TransformPos[0] = pCenter[0];
	TransformPos[1] = pCenter[1];
	TransformPos[2] = pCenter[2];
	QString StringTranslatedPosX = QString::number(TransformPos[0]);
	QString StringTranslatedPosY = QString::number(TransformPos[1]);
	QString StringTranslatedPosZ = QString::number(TransformPos[2]);

	double* pRotation = theApp.ObjectPanelActors[id]->GetOrientation();
	double TransformRot[3];
	TransformRot[0] = pRotation[0];
	TransformRot[1] = pRotation[1];
	TransformRot[2] = pRotation[2];
	QString StringRotX = QString::number(TransformRot[0]);
	QString StringRotY = QString::number(TransformRot[1]);
	QString StringRotZ = QString::number(TransformRot[2]);

	if (pRt->m_ObjectType[id] == 0) // Box
	{
		pRt->ObjectBox_CenterX_QLineEdit->setText(StringTranslatedPosX);
		pRt->ObjectBox_CenterY_QLineEdit->setText(StringTranslatedPosY);
		pRt->ObjectBox_CenterZ_QLineEdit->setText(StringTranslatedPosZ);

		pRt->ObjectBox_RotateX_QLineEdit->setText(StringRotX);
		pRt->ObjectBox_RotateY_QLineEdit->setText(StringRotY);
		pRt->ObjectBox_RotateZ_QLineEdit->setText(StringRotZ);
	}
	if (pRt->m_ObjectType[id] == 1) // Sphere
	{
		pRt->ObjectSphere_CenterX_QLineEdit->setText(StringTranslatedPosX);
		pRt->ObjectSphere_CenterY_QLineEdit->setText(StringTranslatedPosY);
		pRt->ObjectSphere_CenterZ_QLineEdit->setText(StringTranslatedPosZ);

		pRt->ObjectSphere_RotateX_QLineEdit->setText(StringRotX);
		pRt->ObjectSphere_RotateY_QLineEdit->setText(StringRotY);
		pRt->ObjectSphere_RotateZ_QLineEdit->setText(StringRotZ);
	}
	if (pRt->m_ObjectType[id] == 2) // Cylinder
	{
		pRt->ObjectCylinder_CenterX_QLineEdit->setText(StringTranslatedPosX);
		pRt->ObjectCylinder_CenterY_QLineEdit->setText(StringTranslatedPosY);
		pRt->ObjectCylinder_CenterZ_QLineEdit->setText(StringTranslatedPosZ);

		pRt->ObjectCylinder_RotateX_QLineEdit->setText(StringRotX);
		pRt->ObjectCylinder_RotateY_QLineEdit->setText(StringRotY);
		pRt->ObjectCylinder_RotateZ_QLineEdit->setText(StringRotZ);
	}
}
