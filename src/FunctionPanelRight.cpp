...
void FunctionPanelRight::MultipleUICloseTrigger()
{
	auto* OVWidget = getSourceWidget<ObjectVolumeWidget>();
	if(!OVWidget) return;

	if (m_phantoms->getModel().m_Is_PhantomSetting_OKbutton_Clicked == false) m_phantoms->getModel().m_Is_PhantomSetting_Closed = true;
	if (m_phantoms->getModel().m_Is_ClothingSetting_OKbutton_Clicked == false) m_phantoms->getModel().m_Is_ClothingSetting_Closed = true;
	if (m_phantoms->getModel().m_Is_ClothingLayer_Setting_OKbutton_Clicked == false) m_phantoms->getModel().m_Is_ClothingLayer_Setting_Closed = true;
	if (b_IsObjectSettingOKClicked == false) b_IsObjectSettingOKClicked = true;		
...
void FunctionPanelRight::CreateWidgets()
{
	...
	mainLayout->addWidget(m_phantoms);	
	mainLayout->addWidget(SourcePanel_QGroupBox);	
	mainLayout->addWidget(OutputPanel_QGroupBox);	

	setLayout(mainLayout);
	theApp.LoadingBar->setFormat("mainLayout");
	theApp.LoadingBar->setValue(95);

	GeometryPanel_QGroupBox->show();
	//PhantomPanel_QGroupBox->hide();	// 이 부분
	m_phantoms->hide();	// 이 부분
...
void FunctionPanelRight::slot_Phantom_ButtonClicked()
{
	...
	for (auto itr_phantom : m_phantoms->getModel().m_Phantom_SequenceVector) // 최대 팬텀 개수
	{
		if (itr_phantom == m_phantoms->getModel().m_Phantom_SelectedIndex) // 선택된 팬텀버튼
		{
			m_phantoms->getQPushButtonMap(PhantomWidget::ButtonType::Phantom)[itr_phantom]->setChecked(true);			
		}
		else // 나머지 팬텀버튼
		{
			m_phantoms->getQPushButtonMap(PhantomWidget::ButtonType::Phantom)[itr_phantom]->setChecked(false);
			
		}
	}
	if (m_phantoms->getModel().m_Phantom_SelectedIndex != -1) // 생성된 팬텀이 있을 때
	{
		m_phantoms->UpdatePhantom_InfoStatus(m_phantoms->getModel().m_Phantom_SelectedIndex);

		// Wearable 정보 불러오기 

		////////////////////////////////////////////////Wearable 초기화 		

		theApp.UpdatePhantom_ActorHighlighted(m_phantoms->getModel().m_Phantom_SelectedIndex); // 다시 팬텀패널로 돌아왔을때 현재 선택된 팬텀만 pickable and highlight
	}
...
{	
	...
	theApp.UpdatePhantom_ActorHighlighted(PhantomConstants::const_Phantom_MaximumCount); // 전부 highlight and pickable 끄기
}
void FunctionPanelRight::slot_Source_ButtonClicked()
{
	...
	{
		if (m_phantoms->getModel().m_Phantom_SequenceVector.size() != 1)
		{
		...
		{
			theApp.UpdatePhantom_ActorHighlighted(m_phantoms->getModel().m_Phantom_SequenceVector[0]);
		}
	}
...
void FunctionPanelRight::slot_Output_ButtonClicked()
{
	...
	{
		if (m_phantoms->getModel().m_Phantom_MainInfo[m_phantoms->getModel().m_Phantom_SelectedIndex][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES) // phantom 패널에서 더미팬텀을 선택하고 있을때
		{
			for (auto itr_phantomID : m_phantoms->getModel().m_Phantom_SequenceVector)
			{
				if (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomID][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES)
				{
		...
		else OutputPhantomSelectedIndex = m_phantoms->getModel().m_Phantom_SelectedIndex; //팬텀패널에서 풀팬텀 선택하고 있으면 그대로 아웃풋팬텀 인덱스
		
		for (auto phantomID: m_phantoms->getModel().m_Phantom_SequenceVector) // 최대 팬텀 개수
		{
...
{
	m_phantoms = new PhantomWidget(this);

	m_phantoms->panel = this;

	m_phantoms->initialize();
}
...
void FunctionPanelRight::slot_SkinDoseVisualization_ButtonClicked()
{
	if (m_skinDoseVisualizationButton->isChecked()) // Visualization ON
	{
		...
		for (auto phantomID : m_phantoms->getModel().m_Phantom_SequenceVector)
		{
			if (m_phantoms->getModel().m_Phantom_MainInfo[phantomID][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES) continue;
			if (m_phantoms->getModel().m_Phantom_MainInfo[phantomID][E_PHANTOMMAININFO_CATEGORY] == E_PHANTOMCATEGORY_AIR) continue;
...
void FunctionPanelRight::CreateOutput_Dosimeter(QVBoxLayout* layout)
{
...

	{
		...
		for (int i = 0; i < PhantomConstants::m_Dosimeter_Maximum_Count; i++) {
			{
				QHBoxLayout* hLayout = new QHBoxLayout;
				hLayout->setContentsMargins(Window_width * 0.003, Window_height * 0.002, Window_width * 0.003, Window_height * 0.002);
				hLayout->setAlignment(Qt::AlignLeft | Qt::AlignTop);
...
void FunctionPanelRight::SetResultType(int index)
{
	if (0 == index) // Organ/eff dose -> 실시간
	{
		...
		for (auto itr_phantomID : m_phantoms->getModel().m_Phantom_SequenceVector)
		{
			if (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomID][E_PHANTOMMAININFO_CATEGORY] == E_PHANTOMCATEGORY_AIR) OutputPhantomButton[itr_phantomID]->setDisabled(true);
			if (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomID][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES) OutputPhantomButton[itr_phantomID]->setDisabled(true);
		}
	}
		...
		for (auto itr_phantomID : m_phantoms->getModel().m_Phantom_SequenceVector)
		{
			...
			if (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomID][E_PHANTOMMAININFO_CATEGORY] == E_PHANTOMCATEGORY_HUMAN) OutputPhantomButton[itr_phantomID]->setDisabled(true); // Human phantom Disable
		}
	}
...
void FunctionPanelRight::PhantomList_button_generate()
{
	...
	for (auto itr_phantomIndex : m_phantoms->getModel().m_Phantom_SequenceVector)
	{
		OutputPhantomButton[itr_phantomIndex] = new QPushButton(m_phantoms->getModel().m_Phantom_ButtonName[itr_phantomIndex]);
		...
		if (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES || m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][E_PHANTOMMAININFO_CATEGORY] == E_PHANTOMCATEGORY_AIR) OutputPhantomButton[itr_phantomIndex]->setDisabled(true);
...
void FunctionPanelRight::Create_H_AK_BarGraphWidgets(QVBoxLayout* layoutIn_graph) // called by (1) Calculate_Local, (2) LoadReconsturctionFile_previous
{
	...
	H_AK_BarGraph_QCustomPlot->setFixedSize(Window_width * 0.780 * m_phantoms->getModel().m_Phantom_SequenceVector.size() / 7.5, Window_height * 0.244 * SpecialWidgetSizeFactor);
	...
	for (auto itr_phantomID : m_phantoms->getModel().m_Phantom_SequenceVector)
	{
		H_AK_names.push_back(m_phantoms->getModel().m_Phantom_ButtonName[itr_phantomID] + "_H*(10)");
		H_AK_names.push_back(m_phantoms->getModel().m_Phantom_ButtonName[itr_phantomID] + "_AK");
	}
...
{
	int phantomID = m_phantoms->getModel().m_Phantom_SequenceVector[0]; // Only for one phantom
	for (int skindose_type = 0; skindose_type < 2; skindose_type++)
	{
...
void FunctionPanelRight::Write_TextResultFile_SimulationInfo(std::string filepath)
{
	...
	m_phantoms->WritePhantomInfo(ofp_info);
...
void FunctionPanelRight::Gather_PhantomData_mcsee_File(QJsonArray& outPhantomsArray, QJsonObject& outManifestObject, QStringList& outOrderedResourceIds, QMap<QString, QString>& outResourcePathMap, qint64& inoutCurrentOffset)
{
	for (auto itr_phantomIndex : m_phantoms->getModel().m_Phantom_SequenceVector)
	{
		...
		phantomObject[JsonKeys::Phantom::Name] = m_phantoms->getModel().m_Phantom_ButtonName[itr_phantomIndex];

		// Air Sphere인 경우 위치 정보만 저장하고 다음 팬텀으로 넘어감
		if (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][E_PHANTOMMAININFO_CATEGORY] == E_PHANTOMCATEGORY_AIR)
		{
			phantomObject[JsonKeys::Phantom::Category] = "Air"; 
			phantomObject[JsonKeys::Phantom::PosX_cm] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][6];
			phantomObject[JsonKeys::Phantom::PosY_cm] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][7];
			phantomObject[JsonKeys::Phantom::PosZ_cm] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][8];
		...
		int phantomType = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][2];
		...
		phantomObject[JsonKeys::Phantom::IsDummy] = (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES);
		phantomObject[JsonKeys::Phantom::Gender] = (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][1] == 0) ? "Male" : "Female";

		const QMap<int, QString> ageMap = { {0, "Adult"}, {1, "15y"}, {2, "10y"}, {3, "5y"}, {4, "1y"}, {5, "0y"} };
		phantomObject[JsonKeys::Phantom::Age] = ageMap.value(m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][12]);

		const QMap<int, QString> postureMap = { {0, "Standing"}, {1, "Bending"}, {2, "Kneeling"}, {3, "Sitting"}, {4, "Squatting"}, {5, "Walking"} };
		phantomObject[JsonKeys::Phantom::Posture] = postureMap.value(m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][3]);

		phantomObject[JsonKeys::Phantom::Height_cm] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][4];
		phantomObject[JsonKeys::Phantom::Weight_kg] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][5];
				
		phantomObject[JsonKeys::Phantom::PosX_cm] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][6];
		phantomObject[JsonKeys::Phantom::PosY_cm] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][7];
		phantomObject[JsonKeys::Phantom::PosZ_cm] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][8];

		phantomObject[JsonKeys::Phantom::RotX_deg] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][9];
		phantomObject[JsonKeys::Phantom::RotY_deg] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][10];
		phantomObject[JsonKeys::Phantom::RotZ_deg] = m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomIndex][11];
		
		// Clothing 정보 (배열 안에 객체들을 넣는 구조)
		QJsonArray clothingArray;
		for (auto itr_clothingIndex : m_phantoms->getModel().m_Clothing_SequenceVector[itr_phantomIndex])
		{
			QJsonObject clothingObject;

			const QMap<int, QString> clothingTypeMap = {
				{0, m_phantoms->getModel().m_Clothing_ButtonName[itr_phantomIndex][itr_clothingIndex]}, {1, "Upper_head"}, {2, "Upper_arm"},
				{3, "Lower_arm"}, {4, "Hands"}, {5, "Front_trunk"}, {6, "Back_trunk"}, {7, "Pelvis"}, {8, "Upper_leg"},
				{9, "Lower_leg"}, {10, "Feet"}
			};
			clothingObject[JsonKeys::Clothing::Name] = clothingTypeMap.value(m_phantoms->getModel().m_Clothing_Type[itr_phantomIndex][itr_clothingIndex]);

			QJsonArray layersArray;
			for (auto itr_clothingLayerIndex : m_phantoms->getModel().m_ClothingLayer_SequenceVector[itr_phantomIndex][itr_clothingIndex])
			{
				QJsonObject layerObject;
				layerObject[JsonKeys::Layer::Thickness_cm] = m_phantoms->getModel().m_Clothing_MainInfo[itr_phantomIndex][itr_clothingIndex][itr_clothingLayerIndex][0];
				layerObject[JsonKeys::Layer::Density_g_cm3] = m_phantoms->getModel().m_Clothing_MainInfo[itr_phantomIndex][itr_clothingIndex][itr_clothingLayerIndex][1];

				int materialType = m_phantoms->getModel().m_Clothing_MainInfo[itr_phantomIndex][itr_clothingIndex][itr_clothingLayerIndex][2];
...
void FunctionPanelRight::Load_commit_mcsee_FileData(const SceneData& sceneData)
{
	...
	for (const PhantomData& pData : sceneData.phantoms)
	{
		...
		m_phantoms->PhantomAddingSetting_Create();

		// Phantom Adding Setting의 인자 입력 및 OK 버튼 클릭
		m_phantoms->Commit_PhantomSetting(pData);
		m_phantoms->slot_PhantomAddingOK_ButtonClicked();

		// Phantom Add 루틴 실행
		m_phantoms->ExecutePhantomAddRoutines();
	}
...
void FunctionPanelRight::ResetCalculation() // Output 패널 관련 정보를 초기화
{
	...
	for (auto itr_outputButton : m_phantoms->getModel().m_Phantom_SequenceVector)
	{
...
{	
	...
	while (!ifp.eof())
	{
		...
		{
			m_phantoms->SetLoadingBaseDir(dir);
			m_phantoms->ReadPhantomInfo(ifp);
		}
		//Geometry_input
		...
		if (dump == "Simulation_output")
		{
			...
			if (dump == "==========================")
			{
				...
				for (auto itr_phantomID : m_phantoms->getModel().m_Phantom_SequenceVector)
				{
					OutputPhantomButton[itr_phantomID] = new QPushButton(m_phantoms->getModel().m_Phantom_ButtonName[itr_phantomID]);
				...
				std::string DoseStr;
				std::string errorStr;
				for (auto phantomIdx : m_phantoms->getModel().m_Phantom_SequenceVector)
				{
					if (m_phantoms->getModel().m_Phantom_MainInfo[phantomIdx][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES) continue;
					while (ifp >> tmpStr)
					{							
...
void FunctionPanelRight::slot_allButtonClicked()
{
	...
	if (theApp.PanelMode == E_PHANTOM)
	{
		theApp.UpdatePhantom_ActorHighlighted(m_phantoms->getModel().m_Phantom_SelectedIndex); 
...
// == Phantom RAM usage update ==
	...
	for (auto itr_phantomID : m_phantoms->getModel().m_Phantom_SequenceVector)
	{
		if (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomID][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES) whole_dummy_phantom_count++;
		if (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomID][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_NO) whole_computational_phantom_count++;
	...
	////////////////////////////////	
	if (whole_computational_phantom_count == 0) m_phantoms->getPhantomRAMUsageLabel()->setText(QString::fromLocal8Bit("0 GB (0 %)"));//Phantom_RAMusage_QLabel->setText(QString::fromLocal8Bit("0 GB (0 %)"));
	else m_phantoms->getPhantomRAMUsageLabel()->setText(QString::number(memory_usage) + QString::fromLocal8Bit(" GB (") + QString::number(memory_occupied) + QString::fromLocal8Bit("%)"));
	
	//////////// Geometry Panel /////////////////////	


	/////////// Output Panel /////////////////////	
	if (m_phantoms->getModel().m_Phantom_MainInfo[m_phantoms->getModel().m_Phantom_SelectedIndex][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES || m_phantoms->getModel().m_Phantom_MainInfo[m_phantoms->getModel().m_Phantom_SelectedIndex][E_PHANTOMMAININFO_TYPE] == E_PHANTOMTYPE_IMPORTED) // Dummy phantom or imported phantom 이면 skin dose 버튼 비활성화
	{
		m_skinDoseVisualizationButton->setDisabled(true);
	}
...
void FunctionPanelRight::SetPhantomPanelMode()
{
	...
	if(b_IsCalculationExecuted) m_phantoms->getModel().m_Phantom_SelectedIndex = OutputPhantomSelectedIndex;
}
//
...
void FunctionPanelRight::SetOutputPanelMode()
{
	...
	if (m_phantoms->getModel().m_Phantom_MainInfo[m_phantoms->getModel().m_Phantom_SelectedIndex][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_YES) // phantom 패널에서 더미팬텀을 선택하고 있을때
	{
		for (auto itr_phantomID : m_phantoms->getModel().m_Phantom_SequenceVector)
		{
			if (m_phantoms->getModel().m_Phantom_MainInfo[itr_phantomID][E_PHANTOMMAININFO_DUMMY] == E_PHANTOMDUMMY_NO)
			{
				OutputPhantomSelectedIndex = itr_phantomID;
				break;
			}
